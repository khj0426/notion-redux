import { generateNestedDocumentsHTML } from '../../utils/Document/extractDocument';
// 전체 문서 목록을 보여주는 컴포넌트
// 각각의 이벤트는 와부애서 주입하게 설정
import DocumentItemTemplate from '../Template/DocumentItemTemplate';
...
export default function DocumentList({
  ...
  this.setEvent = () => {
    $documentLists.addEventListener('click', (e) => {
      ...
      if ($documentItem) {
        ...
        onDocumentClick(id);
        console.log(e.target.closest('.documentItemWrapper'));
      }
  ...
  this.render = () => {
    $documentLists.innerHTML = DOMPurify.sanitize(`
          ${this.state
            .map(({ id, title, documents }) => {
              let html = '<ul class = `documentItemWrapper` id="' + id + '">';
